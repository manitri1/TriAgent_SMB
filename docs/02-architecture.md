...
1. **`terminal` 호출이 60~120초 사이(고정값 아님, 관찰값: 61.6초/121.9초)에 타임아웃될
   수 있습니다**(`exit 124`). 하위 프로필이 code_execution/skill 로딩까지 하는 복잡한
   작업이면 시간이 걸립니다. **타임아웃 후 결과는 매번 다릅니다** — `order-payment-agent`
   위임 때는 하위 프로세스가 백그라운드에서 계속 실행되어 결과를 완성했지만(`ps aux`로
   고아 프로세스 없음 확인), `inventory-agent` 위임("오늘 브리핑" 시나리오) 때는 프로세스가
   실제로 종료돼 결과가 전혀 남지 않았습니다(`ps aux`에 흔적 없음, workspace 파일도 갱신
   안 됨). **"타임아웃 후에도 완료된다"는 가정은 성립하지 않으므로**, coordinator는
   타임아웃 응답만으로 성공도 실패도 단정하지 말고 반드시 Active Verification(산출물
   재확인)으로 실제 완료 여부를 판단해야 합니다. 두 실측 모두에서 coordinator는 실제로
   이렇게 행동했습니다 — 완료된 건은 검증 파일을 열어 확인 후 보고했고, 완료되지 않은
   건은 "데이터를 받지 못했다"고 있는 그대로 보고하며 재시도 여부를 사용자에게 물었습니다
   (데이터를 지어내지 않음).
