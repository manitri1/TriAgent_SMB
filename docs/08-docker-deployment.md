...
- **게이트웨이:** 호스트 `18651` → 컨테이너 `8642`
- **대시보드:** 호스트 `127.0.0.1:19128` → 컨테이너 `9119`
- **컨테이너명:** `hermes-triagent-smb`, `hermes-triagent-smb-dashboard`
- **Mock POS:** 호스트 `18080` → 컨테이너 `8080`
- **Webapp:** 호스트 `127.0.0.1:19131` → 컨테이너 `8090`

> 2026-09-07 갱신: 위 4개 호스트 포트는 원래 `8651`/`9128`/`8080`/`9131`이었으나, VPS에
> 새로 배포되는 다른 서비스와의 4자리 포트 충돌 가능성을 줄이기 위해 1xxxx 대역으로
> 일괄 이전했다(기존 값 + 10000). 컨테이너 **내부** 포트(8642/9119/8080/8090)는
> 변경하지 않았다 — `docker-compose.yml`과 아래 "VPS 배포 현황" 표를 최신값으로 갱신했다.
...
|---|---|---|---|---|
| **TriAgent_SMB (본 저장소)** | `hermes-triagent-smb*` | `18651`(공개) | `127.0.0.1:19128` | mock-pos `18080`(공개), webapp `127.0.0.1:19131` — 2026-09-07 1xxxx 대역으로 재배치(이전: 8651/9128/8080/9131) |
| adcreator (형제 프로젝트, `/opt/adcreator`) | `hermes-adcreator*` | `8652`(공개) | `127.0.0.1:9130` | 예전엔 `9130`이 본 프로젝트의 옛 웹앱 포트(`9131`)와 한 자리 차이라 혼동됐지만(14장 문서에 `9130`으로 잘못 기재된 적도 있었음), 본 프로젝트가 `19131`로 이전하면서 더는 헷갈릴 여지가 없음 |
